...
public extension Date {
    ...
    func diffrence(with date: Date) -> String {
        let components = Calendar.current.dateComponents([.day, .hour, .minute], from: self, to: date)
        if let day = components.day, day > 0 {
            if let hour = components.hour, hour > 0 {
                return "\(day)d \(hour)h \(components.minute ?? 0)m"
            }else {
                return "\(day)d \(components.minute ?? 0)m"
            }
    ...
    func toString(format: DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = .app) -> String? {
        switch format {
            case .dotNet:
    ...
    enum TimeZoneType {
        case local, `default`, utc, custom(Int), customTimezone(String)
        var timeZone: TimeZone? {
            switch self {
                ...
                case let .customTimezone(abbreviation): return TimeZone(abbreviation: abbreviation)
            }
        }
    }
...
extension Locale {
    public static var api: Locale {
        return Locale(identifier: "en")
    }
    public static var app: Locale {
        return Locale(identifier: AppSettings.selectedLanguageDefualt.instance.id)
    }
}
